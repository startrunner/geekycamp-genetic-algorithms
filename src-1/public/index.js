...
function init() {
    ...
    for (let i = 0; i < world.geneticModel.populationSize; i++) {
        let params = randomParameters(12);
...
function updateSensor(tank, sensor) {
    ...
    if(!sensor.hit)sensor.hitDistance = 0;
}
...
function updateInput() {
    ...
    for (let i = 0; i < world.geneticModel.populationSize; i++) {
        ...
        let parameters = world.geneticModel.individuals[i].parameters;
        movement2(tank, parameters);
    }
...
function update(ctx) {
    //console.log("Updating... with AI: " + world.AI);
    updateHTML();
    updateInput();
...
function updateHTML() {
    ...
    content.innerHTML = 
    `SIMULATION TIME LEFT: ${(world.timeBetweenSimulations - (Date.now() - world.lastResetTime)) / 1000}`;
